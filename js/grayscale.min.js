...
//Noe Firebase Database -- needed to incorporate google hotel api & save recent city searches 
// var config = {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.page.totalElements > 0) {
                ...
                for (var i = 0; i < 10; i++) {
                    var eventDiv = $("<div>");
                    // fix for missing genre info
                    if (results.events[i].classifications[0].genre !== undefined) {
                        var eventDescriptions = $("<p>").text("Type of event: " + results.events[i].classifications[0].genre.name);
                    } else {
                        var eventDescriptions = $("<p>").text('Click the "Buy Tickets" button for updated information.');
                    }
                    ...
                    var eventImage = $("<img>");
                    var eventTicket = $("<a>");
                    // fix for missing pricing info
                    if (results.events[i].priceRanges !== undefined) {
...
// clear previous searches whenever the window reloads
// $(window).on("load", function() {
//     $("#oldSearchResultsContainer").empty();
// });
...
database.ref().on("child_added", function(childSnapshot) {
    ...
    searchedButton.addClass("searchedButton");
    ...
    $(".searchedButton").on("click", function() {
        ...
        //clear events results after user clicks a button
        $("#eventAppend").empty();
